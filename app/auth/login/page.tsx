...
const LoginPage = () => {
  ...
  const { getAllUsers, getAllGroups } = useFirestore();
  const [users, setUsers] = useState<{ id: string; name: string }[]>([]);
  const [filteredUsers, setFilteredUsers] = useState<{ id: string; name: string; groupName: string }[]>([]);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const usersWithGroupNames = allUsers.map(user => ({
          ...user,
          groupName: allGroups.find(group => group.id === user.group_id)?.name // find group name based on user.group_id
        }))
        setUsers(usersWithGroupNames);
        setFilteredUsers(usersWithGroupNames.slice(0, 5));
      } catch (error) {
                    ...
                    <ul className="absolute bottom-2 w-full border-2 border-violet-700 bg-violet-100/90 backdrop-blur-lg rounded-2xl overflow-hidden">
                      {filteredUsers.map((user) => (
                        <li key={user.id} onClick={() => handleUserSelect(user)} className="cursor-pointer p-2 hover:bg-violet-200 transition-colors">
                          {user.name} - <span className='text-gray-500'>{user.groupName}</span>
                        </li>
                      ))}
                    </ul>
                  )}
